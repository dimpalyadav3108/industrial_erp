...
export const getEstimates = async (search = ""): Promise<Estimate[]> => {
  ...
  const response = await fetch(`${API_URL}/estimates${query}`, { headers: headers() });
  const result = await response.json() as EstimateListResponse | ApiErrorResponse;
  if (!response.ok) throw new Error(result.message || "Unable to load estimates");
  return (result as EstimateListResponse).data;
};

export const createEstimate = async (payload: CreateEstimatePayload): Promise<Estimate> => {
  const response = await fetch(`${API_URL}/estimates`, { method: "POST", headers: headers(true), body: JSON.stringify(payload) });
  const result = await response.json() as EstimateResponse | ApiErrorResponse;
  if (!response.ok) throw new Error(result.message || "Unable to create estimate");
  return (result as EstimateResponse).data;
};

export const updateEstimateStatus = async (estimateId: string, status: EstimateStatus): Promise<Estimate> => {
